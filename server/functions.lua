...
function ESX.GetIdentifier(playerId)
    local fxDk = GetConvarInt("sv_fxdkMode", 0)
    if fxDk == 1 then
        return "steam:ESX-DEBUG-LICENCE"
    end
...
if not Config.CustomInventory then
    ...
    function ESX.RefreshItems()
        ...
        for i = 1, itemCount do
            ...
            ESX.Items[item.name] = { label = item.label, limit = item.limit, rare = item.rare, canRemove = item.can_remove }
        end
    ...
    ---@param items { name: string, label: string, limit?: number, rare?: boolean, canRemove?: boolean }[]
    function ESX.AddItems(items)
        ...
        if #toInsert > 0 then
            MySQL.prepare.await(
            -- "INSERT IGNORE INTO `items` (`name`, `label`, `weight`, `rare`, `can_remove`) VALUES (?, ?, ?, ?, ?)", toInsert)
            "INSERT IGNORE INTO `items` (`name`, `label`, `limit`, `rare`, `can_remove`) VALUES (?, ?, ?, ?, ?)", toInsert)
