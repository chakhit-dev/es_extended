...
function CreateExtendedPlayer(playerId, identifier, ssn, group, accounts, inventory, limit, job, loadout, name, coords, metadata)
    ...
    ---@return boolean
    function self.canCarryItem(itemName, count)
        local ItemObject = ESX.Items[itemName]
        
        if ItemObject then
            local currentItem = self.getInventoryItem(itemName)
            local currentCount = currentItem and currentItem.count or 0

            if ItemObject.limit ~= -1 and (currentCount + count) > ItemObject.limit then
                return false
            end
